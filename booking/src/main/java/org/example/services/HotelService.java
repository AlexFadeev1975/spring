...
public class HotelService {
    ...
    public HotelDtoResponse createHotel(HotelDtoRequest request) {
        ...
        List<HotelDtoResponse> savedHotels = searchHotel(null, request.getName(), request.getCity(), request.getAddress(), null, null, null, null, null);
    ...
    public HotelDtoResponse updateHotel(HotelDtoRequest request) {
        ...
        Hotel updatedHotel = mapper.mapDtoToHotel(request);
        ...
        if (updatedHotel.getHeader() != null) {
            hotel.setHeader(updatedHotel.getHeader());
        }
        ...
        if (updatedHotel.getRemotion() != null) {
            hotel.setRemotion(updatedHotel.getRemotion());
        }
    ...
    public List<HotelDtoResponse> searchHotel(Long id, String hotelName, String cityName, String address, String header, Float remotion, Float rating, Integer ratesCount, Pageable pageable) {
        ...
        } else return hotelRepository.findAll(specification, pageable).getContent().stream().map(mapper::mapHotelToDto).toList();
    }
    public HotelDtoResponse changeHotelRating (HotelDtoRequest dto) throws ParseException {
        ...
        Float rating = (hotel.getRating() == 0) ? dto.getMark() : hotel.getRating();

        Integer numberOfRating = (hotel.getRatesCount() == 0) ? 1 : hotel.getRatesCount();
        ...
        rating = (Float) format.parse(format.format(totalRating / numberOfRating));
